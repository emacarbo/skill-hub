...
description: "Use when setting up Claude Code to run autonomously without human input — triggers on \"autonomous workflow\", \"run claude in a loop\", \"continuous development pipeline\", \"RFC-driven agents\", \"multi-agent DAG\", \"chain claude -p calls\", or choosing between a simple pipe and full orchestration. Covers patterns from one-shot loops to multi-agent DAGs. NOTE: superseded by continuous-agent-loop in newer guidance; kept for compatibility."
origin: ECC
---
